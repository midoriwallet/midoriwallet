    ...
    async confirm() {
      ...
      await this.walletSeed(async (walletSeed) => {
        try {
          ...
          if (err instanceof errors.NodeError && this.$wallet.platform.id === 'base') {
            this.updateStorage({
              status: false,
              message: this.$t('Base network is temporarily unavailable. Please try again in a few minutes.'),
            });
            return;
          }
